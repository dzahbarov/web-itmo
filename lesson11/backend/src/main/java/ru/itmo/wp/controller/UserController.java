...
public class UserController {
    ...
    @PostMapping("users")
    public User register(@RequestBody @Valid UserRegisterCredentials userRegisterCredentials, BindingResult bindingResult){
        if (bindingResult.hasErrors()) {
            throw new ValidationException(bindingResult);
        }
        User user = userService.save(userRegisterCredentials);
        return user;
    }
}
