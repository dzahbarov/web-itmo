...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException, NoSuchAlgorithmException {
        ...
        if (!file.isFile()) {
            file = new File(root, request.getUri().concat("/index.html"));
        }

        if (file.isDirectory() || file.isFile()) {
            byte[] body = readFile(file);
            String oldHash = String.valueOf(Arrays.hashCode(body));
            if (oldHash.equals(request.getHeader("If-None-Match"))) {
                response.setHeader("ETag", oldHash);
            ...
            response.setHeader("ETag", String.valueOf(Arrays.hashCode(body)));
        } else {
